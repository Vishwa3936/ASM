...
| `masscan` | 5 — port scan (large sets, `--masscan`/`--masscan-only`) | C | `apt` |
...
```bash
sudo apt-get update
sudo apt-get install -y nmap masscan git curl wget unzip build-essential libpcap-dev \
    python3 python3-venv python3-pip
```
`libpcap-dev` is required to build `naabu`; `build-essential` for the Go tools.
`masscan` powers the fast large-set port scan (`--masscan` / `--masscan-only`) and
needs root at runtime (raw sockets) — run those scans with `sudo`.
...
- **`sudo` + Go tools PATH gotcha.** `sudo` resets PATH/HOME to root's, so the
  Go-installed tools in `~/go/bin` (cdncheck, dnsx, httpx, naabu) can read as "not
  found" even though they work in your normal shell. If cdncheck goes missing,
  Stage 4 can't filter CDN/WAF edges and they get scanned (e.g. Incapsula tarpits
  reporting thousands of fake open ports). NewASM now also probes `~/go/bin` (for
  both `$HOME` and `$SUDO_USER`) and `/usr/local/bin`, but the bulletproof options
  are: install these to `/usr/local/bin`, or run
  `sudo env "PATH=$PATH" python3 asm.py ...`. Watch the log for
  `cdncheck NOT FOUND on PATH`.
