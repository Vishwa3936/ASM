...
def stage_http_probe(ctx: Context, hosts: List[str]) -> Set[str]:
    ...
    stored = 0
    reached: Set[str] = set()
    alive_urls: List[str] = []             # URLs that responded (for fuzzing input)
    fuzz_urls: List[str] = []              # URLs with fuzzable status codes
    ...
    for rec in utils.read_jsonl(text):
        ...
        stored += 1
        url = rec.get("url")
        status = utils.first(rec, "status_code", "status-code")
        if host:
            reached.add(host)   # httpx answered → provably reachable
        if url:
            alive_urls.append(url)
            if isinstance(status, int) and status in (200, 401, 403):
    ...
    # Alive URLs artifact: every URL httpx got a response from, deduplicated,
    # one per line.
    unique_alive = sorted(set(alive_urls))
    utils.write_lines(ctx.stage_file("03_alive_urls.txt"), unique_alive)
    # Fuzz targets: only URLs that returned 200/401/403 — the status codes
    # worth directory-fuzzing (serving content or blocking access).
    unique_fuzz = sorted(set(fuzz_urls))
    utils.write_lines(ctx.stage_file("03_fuzz_targets.txt"), unique_fuzz)

    log.info("Stage 3 complete: %d HTTP services recorded (incl. 404s); "
             "%d host(s) reachable (fed to Stage-4 cross-check); "
             "%d alive URL(s), %d fuzz target(s) written",
             stored, len(reached), len(unique_alive), len(unique_fuzz))
...
def _tarpit_precheck(ctx: Context, ips: List[str],
    ...
    # NOTE: no --min-rate here (unlike the full scan). The precheck is only
    # ~20 ports × N IPs — finishing in seconds regardless. Using --min-rate
    # 10000 caused nmap to blast all probes in <0.1s and close the receive
    # window before slow/high-latency hosts responded, letting tarpits like
    # 92.204.80.20 slip through with 0 open probe ports.
    ctx.runner.run(
        [nmap_bin, "-Pn", "--max-rtt-timeout", "3000ms",
         "--max-retries", "2", "-p", probe_ports,
...
def stage_nmap(ctx: Context, scannable_ips: List[str],
    ...
    with ThreadPoolExecutor(max_workers=ctx.settings.threads_nmap) as pool:
        ...
        for done, fut in enumerate(as_completed(futures), 1):
            ...
            # Post-scan tarpit safety net: if an IP that passed the precheck
            # still reported an absurd number of open ports, it's a tarpit the
            # precheck missed (high-latency host whose SYN-ACKs arrived late).
            # Purge the fake ports from the DB so the report stays clean.
            if ctx.settings.tarpit_detection and ip not in tarpit_ips:
                port_count = ctx.storage.port_count(ctx.run_id, ip)
                if port_count > 500:
                    log.warning("Post-scan tarpit caught: %s — %d open ports "
                                "(precheck missed it). Purging fake ports.",
                                ip, port_count)
                    ctx.storage.purge_ports(ctx.run_id, ip)
                    for cp in config.TARPIT_CURATED_PORTS.split(","):
                        ctx.storage.add_port(ctx.run_id, ip, int(cp.strip()))
                    postscan_tarpits.add(ip)
            log.debug("Stage 6 progress: %d/%d scanned (%d left) — done: %s",
                      done, total, total - done, ip)
    ctx.storage.commit()

    all_tarpits = tarpit_ips | postscan_tarpits
    if postscan_tarpits:
        log.warning("Post-scan tarpit safety net caught %d IP(s) the precheck "
                     "missed: %s", len(postscan_tarpits),
                     ", ".join(sorted(postscan_tarpits)))
    utils.dump_json(ctx.stage_file("06_nmap.json"),
                    {"scanned_hosts": len(targets),
                     ...
                     "tarpit_ips": sorted(all_tarpits),
                     "tarpit_precheck": sorted(tarpit_ips),
                     "tarpit_postscan": sorted(postscan_tarpits),
                     "tarpit_curated_ports": config.TARPIT_CURATED_PORTS
                     if all_tarpits else None})
    log.info("Stage 6 complete: deep-scanned %d host(s) (port scope: %s)%s",
             len(targets), scope_desc,
             f" — {len(all_tarpits)} tarpit(s) detected "
             f"({len(tarpit_ips)} precheck, {len(postscan_tarpits)} post-scan)"
             if all_tarpits else "")
...
# ---------------------------------------------------------------------------
# Stage 7 — feroxbuster directory fuzzing (opt-in via --fuzz).
#
# Lightweight content discovery on fuzz-worthy URLs (200/401/403 from Stage 3)
# whose resolved IPs are scannable (not CDN edges). Per-URL invocations so each
# target gets its own --filter-similar-to baseline (soft-404 / wildcard page
# filtering). Conservative defaults: quickhits.txt (~2.5K entries), no recursion,
# rate-limited, auto-tuned — discovers interesting paths without stressing the
# target. Runs AFTER nmap so a ^C during fuzzing preserves all scan results.
# ---------------------------------------------------------------------------
def _ferox_filename(url: str) -> str:
    """Derive a safe, readable filename from a URL for raw output."""
    parsed = urlparse(url)
...
def stage_feroxbuster(ctx: Context) -> None:
    ...
    ferox_bin = ctx.runner.resolve_binary(config.TOOL_BINARIES["feroxbuster"])
    if not ferox_bin:
        log.warning("feroxbuster not found on PATH — skipping directory fuzzing")
        return

    wordlist = Path(ctx.settings.ferox_wordlist)
    if not wordlist.exists():
        log.error("feroxbuster wordlist not found: %s — skipping directory fuzzing. "
                  "Install seclists: apt install seclists", wordlist)
        return
    ...
    log.info("Stage 7: fuzzing %d URL(s) with feroxbuster (threads=%d, "
             "rate-limit=%d req/s, time-limit=%s per URL, wordlist=%s, "
             "no recursion, %d concurrent URLs)",
             len(targets), ctx.settings.ferox_threads,
             ctx.settings.ferox_rate_limit, ctx.settings.ferox_time_limit,
             wordlist.name, ctx.settings.threads_ferox)

    all_results: List[Dict[str, Any]] = []

    def fuzz_one(url: str) -> List[Dict[str, Any]]:
        out_path = ctx.raw("feroxbuster", _ferox_filename(url))
        ctx.runner.run(
            [ferox_bin, "-u", url,
             "--wordlist", str(wordlist),
             "--threads", str(ctx.settings.ferox_threads),
             "--rate-limit", str(ctx.settings.ferox_rate_limit),
             "--auto-tune",
             "--filter-status", config.FEROX_FILTER_STATUS,
             "--filter-similar-to", url,
             "-n",
             "--time-limit", ctx.settings.ferox_time_limit,
             "--dont-extract-links",
             "-H", f"User-Agent: {config.FEROX_USER_AGENT}",
             "--output", str(out_path),
             "--json",
             "--no-state",
             "--insecure"],
            timeout=config.TIMEOUTS["feroxbuster"],
        )
        return _parse_ferox_json(out_path, url)

    def _parse_ferox_json(path: Path, target_url: str) -> List[Dict[str, Any]]:
        if not path.exists():
            return []
    ...
    total = len(targets)
    with ThreadPoolExecutor(max_workers=ctx.settings.threads_ferox) as pool:
        futures = {pool.submit(fuzz_one, url): url for url in targets}
        for done, fut in enumerate(as_completed(futures), 1):
            url = futures[fut]
            try:
                results = fut.result()
            except Exception as exc:
                log.warning("feroxbuster failed for %s: %s", url, exc)
                results = []
            all_results.extend(results)
            for r in results:
                ctx.storage.add_fuzz_result(
                    ctx.run_id, r["target_url"], r["found_url"],
                    r.get("status"), r.get("content_length"),
                )
            log.debug("Stage 7 progress: %d/%d fuzzed (%d left) — done: %s (%d paths)",
                      done, total, total - done, url, len(results))
    ctx.storage.commit()

    ok_urls = sorted({r["found_url"] for r in all_results if r.get("status") == 200})
    utils.write_lines(ctx.stage_file("07_fuzz_200_urls.txt"), ok_urls)

    utils.dump_json(ctx.stage_file("07_fuzz_results.json"), {
        "targets_fuzzed": total,
        "paths_found": len(all_results),
        "paths_200": len(ok_urls),
        "wordlist": str(wordlist),
        "settings": {
            "threads": ctx.settings.ferox_threads,
            "rate_limit": ctx.settings.ferox_rate_limit,
            "time_limit": ctx.settings.ferox_time_limit,
            "recursion": False,
        },
        "results": all_results,
    })
