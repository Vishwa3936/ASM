...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--httpx-threads", type=int, default=config.Settings.httpx_threads,
                   help="httpx-toolkit concurrent threads (-t). Lower to reduce burst "
                        "when many hosts share a WAF/CDN IP")
    p.add_argument("--httpx-rate-limit", type=int,
                   default=config.Settings.httpx_rate_limit,
                   help="httpx-toolkit max requests/sec (-rl). Controls how fast "
                        "targets are probed — keep low to avoid WAF rate-limiting")
    ...
    p.add_argument("--fuzz", action="store_true",
                   help="Enable feroxbuster directory fuzzing (Stage 7) on fuzz-worthy "
                        "URLs (200/401/403) whose IPs are scannable. Lightweight: "
                        "quickhits.txt, no recursion, rate-limited, auto-tuned")
    p.add_argument("--ferox-threads", type=int, default=config.Settings.ferox_threads,
                   help="feroxbuster internal threads per URL (controls concurrency "
                        "of requests to a single target)")
    p.add_argument("--ferox-rate-limit", type=int,
                   default=config.Settings.ferox_rate_limit,
                   help="feroxbuster max requests/sec per URL (the primary "
                        "lightweight knob — keep low to avoid stressing targets)")
    p.add_argument("--ferox-time-limit", default=config.Settings.ferox_time_limit,
                   help="feroxbuster per-URL wall-clock cap (e.g. '5m', '2m')")
    p.add_argument("--ferox-wordlist", default=config.FEROX_DEFAULT_WORDLIST,
                   help="Wordlist for feroxbuster directory fuzzing")
    p.add_argument("--threads-ferox", type=int, default=config.Settings.threads_ferox,
                   help="Concurrent URLs being fuzzed simultaneously (aggregate "
                        "rate = this × --ferox-rate-limit)")

    p.add_argument("--nuclei", "--cve-scan", action="store_true", dest="nuclei",
                   help="Enable Nuclei CVE + misconfiguration scanning (Stage 8) "
                        "using tech fingerprints from httpx (Stage 3). Runs targeted "
                        "templates based on detected technologies. Conservative: no "
                        "DoS/fuzz/intrusive, rate-limited, critical+high+medium only")
    p.add_argument("--nuclei-rate-limit", type=int,
                   default=config.Settings.nuclei_rate_limit,
                   help="Nuclei max requests/sec per host (-rl)")
    p.add_argument("--nuclei-concurrency", type=int,
                   default=config.Settings.nuclei_concurrency,
                   help="Nuclei internal concurrency per host (-c)")
    p.add_argument("--nuclei-severity", default=config.NUCLEI_SEVERITY,
                   help="Nuclei severity filter (comma-separated)")
    p.add_argument("--nuclei-timeout", default=config.Settings.nuclei_timeout,
                   help="Nuclei per-host wall-clock timeout (e.g. '10m', '5m')")
    p.add_argument("--threads-nuclei", type=int,
                   default=config.Settings.threads_nuclei,
                   help="Concurrent hosts being CVE-scanned simultaneously")
    p.add_argument("--nuclei-extra-tags", default="",
                   help="Additional nuclei template tags to include (comma-separated, "
                        "added to the tech-derived tags)")
...
def main() -> int:
    ...
    settings = config.Settings(
        ...
        httpx_rate_limit=args.httpx_rate_limit,
        run_feroxbuster=args.fuzz,
        ferox_threads=args.ferox_threads,
        ferox_rate_limit=args.ferox_rate_limit,
        ferox_time_limit=args.ferox_time_limit,
        ferox_wordlist=args.ferox_wordlist,
        threads_ferox=args.threads_ferox,
        ...
        threads_nuclei=args.threads_nuclei,
        nuclei_extra_tags=args.nuclei_extra_tags,
