...
# whose IPs are scannable (not CDN edges). Per-URL invocations so each gets its
# own --filter-similar-to baseline (catches soft-404 / wildcard pages). Kept
# deliberately conservative: small wordlist, no recursion, rate-limited, auto-
# tuned — the goal is to discover interesting paths without stressing the target.
# ---------------------------------------------------------------------------
FEROX_DEFAULT_WORDLIST: str = "/usr/share/seclists/Discovery/Web-Content/quickhits.txt"
FEROX_FILTER_STATUS: str = "404,429,400,501"
...
class Settings:
    ...
    ferox_rate_limit: int = 20           # requests/sec per URL
